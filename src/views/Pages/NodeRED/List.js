...
class List extends Component{
  constructor() {
    ...
    render() {
        if(this.props.details.namered === undefined) this.item = "";
      else {
          this.item = this.props.details.namered.map((item, key) =>
          
          <tr>
           
           <td><button disabled="true" aria-pressed="true" class="btn btn-dark btn-block active">{item.namered}</button></td>
           <td>
           <ListStatus namecontainer={item.namered} />
           </td>
           <td>
           <ListPort namecontainer={item.namered}/>
           </td>
           <td>
           <ListPortui namecontainer={item.namered}/>
           </td>
           <td width="250px">
           <Listname name={item.namered} /></td>
           </tr>
...
class ListStatus extends Component{
  ...
  getcontainer() {
    ...
    let url = 'http://192.168.56.103:3000/nameredd/getstatuscontainer';
    fetch(url, {
      method: "POST",
...
class ListPort extends Component{
  ...
  getcontainer() {
    ...
    let url = 'http://192.168.56.103:3000/nameredd/getportcontainer';
    fetch(url, {
      method: "POST",
      body: JSON.stringify(dataToSend),
      headers: {
...
class ListPortui extends Component{
  constructor() {
    super();
    this.getcontainer = this.getcontainer.bind(this);
    this.state = {
  ...
  getcontainer() {
    ...
    setTimeout(() => {
      ...
      let url = 'http://192.168.56.103:3000/nameredd/getportuicontainer';
      fetch(url, {
        method: "POST",
        body: JSON.stringify(dataToSend),
        headers: {
      ...
      .then(responseJson => {
        ...
        console.log("log port ui: "+responseJson.data)
      })
    }, 1500)
...
class Listname extends Component{
  ...
  runcontainer() {
    let dataToSend = {
      userData: {
        username: localStorage.getItem("USERS_USERNAME"), 
        namered: this.props.name
      },

    };
    let url = 'http://192.168.56.103:3000/nameredd/runcontainer';
    fetch(url, {
      method: "POST",
      body: JSON.stringify(dataToSend),
      headers: {
...
test(){
  ...
  deletecontainner() {
    ...
    let url = 'http://192.168.56.103:3000/nameredd/deletecontainner';
    fetch(url, {
      method: "POST",
      body: JSON.stringify(dataToSend),
      headers: {
  ...
  stopcontainner() {
    ...
    let url = 'http://192.168.56.103:3000/nameredd/stopcontainner';
    fetch(url, {
      method: "POST",
      body: JSON.stringify(dataToSend),
      headers: {
           ...
           <button className="btn btn-danger" onClick={this.toggledelete}><i class="fa fa-trash">  Delete </i></button>
            ...
            <Modal isOpen={this.state.modalpoprun} toggle={this.togglerun} className={this.props.className}>
                  ...
                  <Container>
                      ...
                      <InputGroup className="mb-8">
                       <FormGroup row>
                          <div><Label sm={12}>Run Container Name  {this.props.name} </Label>{''}</div>
                      </FormGroup>
                      </InputGroup>       
                    </div>
            ...
            <Modal isOpen={this.state.modalpopstop} toggle={this.togglestop} className={this.props.className}>
                  ...
                  <Container>
                      ...
                      <InputGroup className="mb-8">
                       <FormGroup row>
                          <div><Label sm={12}>Stop Container Name  {this.props.name} </Label>{''}</div>
                      </FormGroup>
                      </InputGroup>       
                    </div>
                    <div className="modal-footer">
            ...
            <Modal isOpen={this.state.modalpopdelete} toggle={this.toggledelete} className={this.props.className}>
                  ...
                  <Container>
                      ...
                      <InputGroup className="mb-8">
                       <FormGroup row>
                          <div><Label sm={12}>Delete Container Name  {this.props.name} </Label>{''}</div>
                      </FormGroup>
                      </InputGroup>       
                    </div>
                    <div className="modal-footer">
